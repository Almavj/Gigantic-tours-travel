...
const AITravelQuiz = ({ isOpen, onClose }: AITravelQuizProps) => {
  ...
  const getRecommendations = () => {
    ...
    if (interests && interests.length > 0) {
      ...
      .filter((dest): dest is Destination & { personalizedScore: number } => (dest as Destination & { personalizedScore: number }).personalizedScore > 0)
          ...
          <div className="absolute inset-0 overflow-hidden pointer-events-none">
            {floatingElements.map((element, index) => (
              <motion.div
                ...
                style={element.style}
                animate={element.animation as any}
              >
                <img
                  src={element.src}
                  alt=""
                  width={parseInt(element.style.width as string)}
                  height={parseInt(element.style.height as string)}
                            ...
                            <div className="p-4 sm:p-6 flex-1">
                              ...
                              <div className="flex justify-between items-center">
                                <div className="flex items-center text-sm text-white/70">
                                  ...
                                  {rec.location?.address || `${rec.location?.city || ''}, ${rec.location?.country || ''}`}
                                </div>
